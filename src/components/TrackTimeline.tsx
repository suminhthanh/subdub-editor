...
const TrackTimeline: React.FC<TrackTimelineProps> = ({ tracks, currentTime, onTimeChange, onEditTrack, isDubbingService, onTrackChange, showSpeakerColors, isMediaFullyLoaded = true }) => {
  const { t } = useTranslation();
  const [zoomLevel, setZoomLevel] = useState(DEFAULT_ZOOM); // pixels per second
  const scrollContainerRef = useRef<HTMLDivElement>(null);

  const handleTrackChange = (index: number, updatedTrack: Track, recreateAudio: boolean) => {
    onTrackChange(index, updatedTrack, recreateAudio);
  };
        ...
        <Timeline id="track-timeline" style={{ width: `${timelineWidth}px` }}>
          ...
          {tracks.filter(track => !track.deleted).map((track, index) => (
            <TrackRow key={track.id}>
              <TrackItem
                track={track}
                onChange={(updatedTrack, recreateAudio) => handleTrackChange(index, updatedTrack, recreateAudio)}
                zoomLevel={zoomLevel}
                onEdit={onEditTrack}
